...
// Sets all the labels to the correct text based on previous data entered
- (void)viewDidLoad {
    [super viewDidLoad];
    
    _signaturePicture.backgroundColor = [UIColor colorWithRed:236.0/255 green:236.0/255 blue:236.0/255 alpha:1];
    
    /*if(__017SpringChoice == TRUE){
        
        _textParagraph1.text = @"The 2017 Spring Semester will begin the week of January 2, 2017 and end the week of May 22, 2017 (20 weeks).  The calendar, including breaks and holidays, is available on our website www.QuarterNoteMusic.com.   Mid-semester pro-rates are available.";
        
    } else */if (__017FallChoice == TRUE){
        
        _textParagraph1.text = @"The 2017 Fall Semester will begin the week of July 24th, 2017 and end the week of December 11, 2017 (20 weeks). The calender, including breaks and holidays, is available on our website www.QuarterNoteMusic.com. Mid-Semester pro-rates are available.";
        
    }
    
    _paymentString1 = @"Paying in Full.";
    _paymentString2 = @"Paying with a Draft.";
    _paymentString3 = @"Paying with a Deposit.";
    _studentNameLabel.text = _studentNameString1;
    _parentNameLabel.text = _parentNameString1;
    _emailLabel.text = _emailString1;
    if(_payFullTrue1 == TRUE) { _paymentLabel.text = _paymentString1; } else if (_payDraftTrue1 == TRUE){ _paymentLabel.text = _paymentString2; } else if (_payDepositTrue1 == TRUE){ _paymentLabel.text = _paymentString3; }
...
// Goes back to entering info. Keeps everything
-(IBAction)changeInfo:(id)sender{
    
    _changeInfoBool = TRUE;

    [self performSegueWithIdentifier:@"passSegue2" sender:self];
    
}

// Creates a PDF with all the data and picture of signature
-(void) MakeAPDF{
...
// Creates email and opens email dialog
-(void) callMethods{
...
// If they sign up for newsletter put their name in the text file, along with email
// Also puts the PDF into the correct folder based on Instructor
-(void) writeFile{
    ...
    NSString *text = [NSString stringWithFormat:@"%@", _emailString1];
    NSString *textFileName = [NSString stringWithFormat:@"%@.txt", _studentNameString1];
    NSString *localDir = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *localPath = [localDir stringByAppendingPathComponent:textFileName];
    [text writeToFile:localPath atomically:YES encoding:NSUTF8StringEncoding error:nil];
...
// Sends email.
// One email goes to customer and the other to the owners email
...
// Segues while keeping data stored
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    if([segue.identifier isEqualToString:@"passSegue2"]){
        ...
        if(_startOver == FALSE){
            ...
            if(_payFullTrue1 == TRUE){ if(!controller.turnPayFullOn || !controller.turnPayDraftOn){ controller.turnPayFullOn = TRUE; controller.turnPayDraftOn = FALSE; controller.turnDepositOn = FALSE; } }
            
            if(_payDraftTrue1 == TRUE){ if(!controller.turnPayDraftOn || !controller.turnPayFullOn){ controller.turnPayDraftOn = TRUE; controller.turnPayFullOn = FALSE; controller.turnDepositOn = FALSE; } }
            
            if(_payDepositTrue1 == TRUE){ if(!controller.turnPayFullOn || !controller.turnPayDraftOn || !controller.turnDepositOn){ controller.turnDepositOn = TRUE; controller.turnPayFullOn = FALSE; controller.turnPayDraftOn = FALSE; }
    ...
    if([segue.identifier isEqualToString:@"passSegue3"]){
        ...
        if(_payFullTrue1 == TRUE){ if(!controller1.payFullTrue || !controller1.payDraftTrue){ controller1.payFullTrue = TRUE; controller1.payDraftTrue = FALSE; controller1.payDepositTrue = FALSE; }
            if(_payDraftTrue1 == TRUE){ controller1.payDraftTrue = TRUE; controller1.payFullTrue = FALSE; controller1.payDepositTrue = FALSE; } if(_payDepositTrue1 == TRUE){ controller1.payDepositTrue = TRUE; controller1.payFullTrue = FALSE; controller1.payDraftTrue = FALSE; } } }
